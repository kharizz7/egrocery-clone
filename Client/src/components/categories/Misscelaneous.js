...
const Miscellaneous = () => {
  ...
  useEffect(() => {
    dispatch(fetchStocks("Miscellaneous"));
  }, [dispatch]);
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6 text-center text-indigo-700">Miscellaneous Items</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {stocks.length > 0 ? (
          ...
          <p className="text-center text-gray-500 col-span-3">No miscellaneous products available</p>
        )}
      </div>
    </div>
  );
